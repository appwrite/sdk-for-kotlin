...
class Avatars(client: Client) : Service(client) {
     ...
     * @param userId User ID to resolve the photo for. Defaults to 'current()' for the currently authenticated user.
     * @param emailHash SHA256 hash of the lowercase, trimmed email address to look up on Gravatar and Libravatar instead of the user's own photo sources. Pass the hash, never the address itself.
     * @param name Name to render initials from instead of the user's own photo sources. Max length: 128 chars.
     * @return [ByteArray]
     */
    @JvmOverloads
    @Throws(AppwriteException::class)
